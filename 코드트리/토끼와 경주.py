...
# #FAIL# 1442 start 1800 end
# 2시간 걸려서 한번 풀어봤는데 시간초과 났음
# 테케 보자마자 아찔해지고 우선순위 큐 사용해야 한다는거 알아서 의지가 꺾여버림
# 답지보고 구조체 더 연습해야겠다 생각함
...
class Rabbit:
    def __init__(self, x, y, j, pid):
        self.x = x
        self.y = y
        self.j = j  # 이게 jump cnt?
        self.pid = pid

    # 이동할 토끼를 결정하기 위해 정렬함수를 만들어줍니다.
    def __lt__(self, other):    # 비교 매직 메소드
        if self.j != other.j:
...
def start_round(inp):
    ...
    for _ in range(k):
        ...
        dis = dist[pid_to_idx[cur_rabbit.pid]]
        ...
        nex_idx = pid_to_idx[nex_rabbit.pid]
        point[nex_idx] = (nex_rabbit.x, nex_rabbit.y)
        jump_cnt[nex_idx] += 1

        # 토끼가 달렸는지 여부를 체크해줍니다.
        movedchk[nex_idx] = True

        # 토끼가 받는 점수는 (현재 뛴 토끼)만 (r + c)만큼 점수를 빼주고,
        # 모든 토끼(total_sum)에게 (r + c)만큼 점수를 더해줍니다.
        # 최종적으로 i번 토끼가 받는 점수는 score[i] + total_sum이 됩니다.
        score[nex_idx] -= (nex_rabbit.x + nex_rabbit.y)
    ...
    while rabbit_pq:
        ...
        # 달리지 않은 토끼는 스킵합니다.
        if not movedchk[pid_to_idx[cur_rabbit.pid]]:
            continue
    ...
    score[pid_to_idx[bonus_rabbit.pid]] += bonus
